...
public class ImageLoader {
     ...
     * 初始化磁盘缓存
     */
    private void initDiskCache(Context ctx) {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(ctx, "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(ctx),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
