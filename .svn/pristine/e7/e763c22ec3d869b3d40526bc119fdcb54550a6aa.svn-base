...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        //获取扩展内容
		AppHelper.getInstance().isVoiceCalling = true;
		callType = 0;
...
//		nickTextView.setText(username);
        ...
        if(username.length()==11){
            String currentUserName = username.substring(0,3)+"****"+username.substring(7,username.length());
            nickTextView.setText(currentUserName);
        }else {
            nickTextView.setText(username);
...
//                                            Toast.makeText(VoiceCallActivity.this, "对方不在线，暂时无法通话！", Toast.LENGTH_SHORT).show();
                                            finish();
                                        }
                                    });
                                }
...
//                                        callStateTextView.setText(st7);
    ...
    protected void onDestroy() {
        AppHelper.getInstance().isVoiceCalling = false;
        stopMonitor();
        super.onDestroy();
    }
