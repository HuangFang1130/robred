 ...
 */
public class CustomRollingView extends FrameLayout implements View.OnClickListener {
    ...
    // 字体颜色
//    private int mTextColor = 0xff0099ff;
    private int mTextColor = Color.GRAY;
//    private int mTextColor = 0xff000000;
    // 点击后字体颜色
    private int mClickColor = 0xff0099ff;
    // 字体大小
    private float mTextSize = DensityUtil.dip2px(UIUtils.getContext(),12);
    // 行间距
    private int mTextPadding = DensityUtil.dip2px(UIUtils.getContext(),5);
    // 画笔
    ...
    public CustomRollingView(Context context) {
        this(context, null);
    }

    public CustomRollingView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // 从xml中获取属性
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.CustomRollingView);
        mTextSize = array.getDimension(R.styleable.CustomRollingView_textSize, mTextSize);
        mTextColor = array.getColor(R.styleable.CustomRollingView_textColor, mTextColor);
        array.recycle();
        // 创建默认显示隐藏动画
    ...
    public void setRollingText(List<HomeWinningResponse.WinningBean> array) {
...
//                TextView textView = createTextView(array.get(mPageSize * i + n).getPrompt(), array.get(mPageSize * i + n).getId());
                TextView textView = createTextView(array.get(mPageSize * i + n).getPrompt(), "0");
    ...
    private LinearLayout createContainer() {
        if (mFrameParams == null) {
            mFrameParams = new LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT);
     ...
     * @return
     */
    private TextView createTextView(String text, String id) {
        if (mLinearParams == null) {
            mLinearParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
